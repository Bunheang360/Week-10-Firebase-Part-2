...
class LibraryViewModel extends ChangeNotifier {
  ...
  Future<void> fetchSong({bool forchFetch = false}) async {
    // 1- Loading state
    ...
    try {
      // 1- Fetch songs
      List<Song> songs = await songRepository.fetchSongs(forchFetch: forchFetch);
  ...
  Future<void> likeSong(LibraryItemData item) async {
    try {
      ...
      await songRepository.songLikes(item.song);
    } catch (e) {
      data = AsyncValue.error(e);
    }
    notifyListeners();
